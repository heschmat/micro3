...
from app.auth import get_current_user, verify_worker_token
...
from app.storage import generate_presigned_download_url, upload_file
...
@router.patch("/videos/{video_id}", response_model=VideoUpdateResponse)
def update_video(
    video_id: str,
    db: Session = Depends(get_db),
...
@router.get("/videos/{video_id}/download", response_model=VideoDownloadResponse)
